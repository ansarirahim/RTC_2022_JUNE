...
//if(SelectedFreqRange[0]==512)
 {
for (int i=0;i<4;i++)
{
FreqChannel[i].count=*(pru0DataMemory_int+2+i);
//mods[sampleno][i]=FreqChannel[i].count;
//mods[sampleno][i]=FreqChannel[i].count;
////if(SelectedFreqRange[]==512 || SelectedFreqRange==256)
{

if((FreqChannel[i].count/MOD1)>0)
{
///mods[sampleno][i]=FreqChannel[i].count%MOD1;
//mods[sampleno][i]=FreqChannel[i].count-MOD1;
//printf("\nless than ch=%d,sampp=%d.. mod1=%d",i,sampleno,mods[sampleno]);
//printf("\n sampleno=%d chan=%d mods!=0=%d",sampleno,i,mods[sampleno][i]);
}
//else
mods[sampleno][i]=FreqChannel[i].count;//-MOD1;
}
...
/*
FreqChannel[0].count=*(pru0DataMemory_int+2);
FreqChannel[1].count=*(pru0DataMemory_int+3);
FreqChannel[2].count=*(pru0DataMemory_int+4);
FreqChannel[3].count=*(pru0DataMemory_int+5);
if((FreqChannel[0].count/MOD1)!=0)
mods[sampleno][0]=FreqChannel[0].count%MOD1;
else
mods[sampleno][0]=FreqChannel[0]-MOD1;
FreqChannel[1].countf((FreqChannel[1].count/MOD1)!=0)
mods[sampleno][1]=FreqChannel[1].count%MOD1;
else
mods[sampleno][1]=FreqChannel[1]-MOD1;
f((FreqChannel[2].count/MOD1)!=0)
mods[sampleno][2]=FreqChannel[2].count%MOD1;
mods[sampleno][3]=FreqChannel[3].count%MOD1;
*/
sampleno=*(pru0DataMemory_int+6);

if(sampleno>sampleold){
for(int  u=0;u<4;u++)
{if(FreqChannel[u].count<10)
Freqinput[u]=-1;
}
